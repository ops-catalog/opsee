...
export default function CatalogItem({ item, onClick }: CatalogItemProps) {
  ...
  return (
    <div 
      className="relative flex flex-col items-center p-4 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-pointer"
      onClick={() => onClick(item)}
    >
      <div className="text-4xl mb-2">{getItemIcon(item.class)}</div>
      <h3 className="text-lg font-medium text-gray-900 text-center mb-1 truncate w-full">
        {item.metadata.name}
      </h3>
